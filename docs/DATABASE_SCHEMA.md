...
| tags | TEXT[] | '{}' | NOT NULL | Topic tags. Auto-generated by manifest-tag-framework, user-editable. |
| archived_at | TIMESTAMPTZ | null | NULL | Soft delete |
| created_at | TIMESTAMPTZ | now() | NOT NULL | |
| updated_at | TIMESTAMPTZ | now() | NOT NULL | Auto-trigger |
...
- `tags` GIN index (tag containment queries for Browse Frameworks)
...
| 035_manifest_enrichment.sql | Add `ai_summary` TEXT and `toc` JSONB columns to `manifest_items` for About This Book enrichment |
| 036_ai_frameworks_tags.sql | Add `tags` TEXT[] column + GIN index to `ai_frameworks` for framework topic tagging |
