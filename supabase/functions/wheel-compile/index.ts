...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!authHeader) {
      return new Response(
        JSON.stringify({ compiled_text: '', error: 'Missing authorization header' }),
    ...
    if (authError || !authUser) {
      return new Response(
        JSON.stringify({ compiled_text: '', error: 'Invalid or expired token' }),
        { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
      );
    }
    const userId = authUser.id;

    const { wheel_id, compile_type, conversation_messages, hub_text, support_role, person_context } =
      await req.json();

    if (!compile_type || !conversation_messages || !hub_text) {
      return new Response(
