...
- **Entry points:** (1) GraduationCap icon button on PersonDetail toolbar, shown only for `has_rich_context && !is_first_mate` people. (2) Card-style button on Crew page (matching Marriage Toolbox aesthetic — icon + "Higgins" title + "Help me talk to someone" description) — opens multi-person select modal (HigginsCrewModal) for selecting one or more crew members before launching Higgins. Only shown when rich-context non-spouse crew members exist and not in Sphere view.
